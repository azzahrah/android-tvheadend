...
public class DvbsubStreamReader extends PlainStreamReader {
    ...
    protected Format buildFormat(int streamIndex, @NonNull HtspMessage stream) {
        List<byte[]> initializationData = new ArrayList<>(4);

        final int compositionId = stream.getInteger("composition_id");
        final int ancillaryId = stream.getInteger("ancillary_id");

        initializationData.add(null); // subtitle subtype
        initializationData.add(new byte[] {(byte) ((compositionId >> 8) & 0xFF), (byte) ((compositionId >> 0) & 0xFF)}); // subtitle compositionId
        initializationData.add(new byte[] {(byte) ((ancillaryId >> 8) & 0xFF), (byte) ((ancillaryId >> 0) & 0xFF)}); // subtitle ancillaryId
